...
public class GraphAdjMatrix extends Graph {
	...
	{
		List<Integer> distance2Vertices = new ArrayList<>();
		
		if(v < 0 || v >= getNumVertices())
		{
			return distance2Vertices;
		}
		
		int[][] multipliedMatrix = this.multiply(adjMatrix, adjMatrix);
		
		
		for(int i = 0; i < multipliedMatrix[v].length; ++i)
		{
			/* Need to clarify, why I need to add multiple times
			 * if there are multiple two hops path to a vertex*/
			
			if(multipliedMatrix[v][i]-- > 0)
			{
				distance2Vertices.add(i);
			}
		}				
